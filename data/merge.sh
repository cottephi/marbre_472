...
for file in $path/*${pattern}1_*.csv ; do
...
for file in $path/*${pattern}2_*.csv ; do
  filename="$(basename "${file}")"
  if [ ! -f ${file} ] ; then
    continue
  fi
...
for file in $path/*${pattern}3_*.csv ; do
  filename="$(basename "${file}")"
  if [ ! -f ${file} ] ; then
    continue
  fi
...
for file in $path/*${pattern}4_*.csv ; do
  filename="$(basename "${file}")"
  if [ ! -f ${file} ] ; then
    continue
  fi
...
for file in $path/*${pattern}5_*.csv ; do
  filename="$(basename "${file}")"
  if [ ! -f ${file} ] ; then
    continue
  fi
